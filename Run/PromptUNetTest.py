...
def prompt_test_make(config,path,device):
    ...
    model = PromptUNet(device,3,3,base_c=config['base_c'],kernels=config['kernels'],attention_kernels=config['attention_kernels'],d_model=config['d_model'],)
...
if __name__ == '__main__':
    ...
    for testset in ['GAMMA','GS1','REFUGE TRAIN']:
        print(testset)
        print('-----')
        model_str = "/data/engs-mlmi1/kebl6872/data/models/PromptUNet_lr_0.0005_bs_10_fs_12_[6_12_24_48]_3_2_5_5]/Checkpoint/seed/953/lr_0.0005_bs_10_lowloss.pth"
        ...
        config = dict(epochs=1000, classes=3, base_c = 12, kernels=[6,12,24,48],attention_kernels = [3,2,5,5],d_model=64,
                          ...
                          testset=testset,seed=401,transform=True,device=device)
