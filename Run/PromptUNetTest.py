...
def make(config,path,device):
    ...
    model = PromptUNet(device,3,3,12)
...
if __name__ == '__main__':
    ...
    config = dict(epochs=1000, classes=3, base_c = 12, kernels=[6,12,24,48], norm_name=norm_name,
                      batch_size=1, learning_rate=lr, dataset="GAMMA",
                      testset='GAMA',architecture=model_name,seed=401,transform=True,device=device)
    model, loader, criterion = make(config,"/home/kebl6872/Desktop/new_data/GAMMA/test/promptunet_batch_lr_0.0005_bs_8_fs_12_[6_12_24_48]/Checkpoint/seed/9/lr_0.0005_bs_8_lowloss.pth",device)
    num_points = 6
    with torch.no_grad():
        ...
        for _,(images,labels) in enumerate(loader):
            ...
            prev_output = torch.from_numpy(np.zeros((config['batch_size'],1,512,512))).to(device)# need to make first input appear to havec come from model so it works w gen_points func
